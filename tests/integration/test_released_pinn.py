...
def test_snapshot_released_pinn_routing_decision(tmp_path):
    """Routing consumes the real verdict. NEED_FEA queues FEA and does not run OpenRadioss."""
    _require_released_bundle()
    settings = Settings(
    ...
    decision = session.query(DecisionRow).filter_by(evaluation_id=row.evaluation_id).first()
